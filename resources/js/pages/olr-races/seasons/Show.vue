...
const activeEntriesCount = computed(() => {
    return props.season.entries.filter(e => e.status !== 'deceased' && e.status !== 'missing').length;
});
...
const addEntry = () => {
    ...
    router.post(`/olr-races/${props.olrRace.id}/seasons/${props.season.id}/entries/bulk`, {
