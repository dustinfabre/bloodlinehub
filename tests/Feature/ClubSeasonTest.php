<?php

namespace Tests\Feature;

use App\Models\Club;
use App\Models\ClubSeason;
use App\Models\ClubSeasonRace;
use App\Models\Pigeon;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Tests\TestCase;

class ClubSeasonTest extends TestCase
{
    use RefreshDatabase;

    protected User $user;
    protected Club $club;
    protected ClubSeason $season;

    protected function setUp(): void
    {
        parent::setUp();
        $this->user = User::factory()->create();
        
        $this->club = Club::create([
            'user_id' => $this->user->id,
            'name' => 'Test Club',
            'description' => 'Test description',
        ]);

        $this->season = ClubSeason::create([
            'club_id' => $this->club->id,
            'name' => '2025 Season',
            'year' => 2025,
        ]);
    }

    public function test_club_season_can_load_entries_without_pigeon_status_column(): void
    {
        $pigeon = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Racing Pigeon',
            'gender' => 'male',
            'status' => 'racing',
            'ring_number' => 'USA-2025-12345',
        ]);

        $this->season->entries()->attach($pigeon->id, [
            'entry_number' => '001',
        ]);

        // This should not throw an error about missing pigeon_status column
        $this->season->load(['entries' => function ($query) {
            $query->select('pigeons.id', 'ring_number', 'personal_number', 'name', 'gender', 'status');
        }]);

        $this->assertCount(1, $this->season->entries);
        $this->assertEquals('racing', $this->season->entries->first()->status);
    }

    public function test_club_season_race_can_load_results_without_pigeon_status_column(): void
    {
        $pigeon = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Racing Pigeon',
            'gender' => 'male',
            'status' => 'racing',
            'ring_number' => 'USA-2025-12345',
        ]);

        $this->season->entries()->attach($pigeon->id, [
            'entry_number' => '001',
        ]);

        $race = ClubSeasonRace::create([
            'club_season_id' => $this->season->id,
            'name' => 'Race 1',
            'release_point' => 'Test Location',
            'distance' => 500,
            'distance_unit' => 'km',
            'race_date' => now(),
        ]);

        $race->results()->attach($pigeon->id, [
            'position' => 1,
            'arrival_time' => now(),
            'speed' => 1500.50,
        ]);

        // This should not throw an error about missing pigeon_status column
        $race->load(['results' => function ($query) {
            $query->select('pigeons.id', 'pigeons.ring_number', 'pigeons.personal_number', 'pigeons.name', 'pigeons.gender', 'pigeons.status');
        }]);

        $this->assertCount(1, $race->results);
        $this->assertEquals('racing', $race->results->first()->status);
    }

    public function test_available_pigeons_query_uses_status_instead_of_pigeon_status(): void
    {
        // Create various pigeons with different statuses
        $racingPigeon = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Racing Pigeon',
            'gender' => 'male',
            'status' => 'racing',
        ]);

        $breedingPigeon = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Breeding Pigeon',
            'gender' => 'female',
            'status' => 'breeding',
        ]);

        $stockPigeon = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Stock Pigeon',
            'gender' => 'male',
            'status' => 'stock',
        ]);

        // This query should use status column, not pigeon_status
        $availablePigeons = Pigeon::where('user_id', $this->user->id)
            ->where('status', 'racing')
            ->whereNotIn('id', $this->season->entries->pluck('id'))
            ->select('id', 'ring_number', 'personal_number', 'name', 'gender', 'color', 'bloodline')
            ->get();

        $this->assertCount(1, $availablePigeons);
        $this->assertEquals($racingPigeon->id, $availablePigeons->first()->id);
    }

    public function test_club_season_controller_show_method_compatible_queries(): void
    {
        $this->actingAs($this->user);

        $pigeon1 = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Racing Pigeon 1',
            'gender' => 'male',
            'status' => 'racing',
            'ring_number' => 'USA-2025-00001',
        ]);

        $pigeon2 = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Racing Pigeon 2',
            'gender' => 'female',
            'status' => 'racing',
            'ring_number' => 'USA-2025-00002',
        ]);

        $pigeon3 = Pigeon::create([
            'user_id' => $this->user->id,
            'name' => 'Stock Pigeon',
            'gender' => 'male',
            'status' => 'stock',
            'ring_number' => 'USA-2025-00003',
        ]);

        $this->season->entries()->attach($pigeon1->id, ['entry_number' => '001']);

        // Simulate the controller's show method queries
        $this->season->load(['entries' => function ($query) {
            $query->select('pigeons.id', 'ring_number', 'personal_number', 'name', 'gender', 'status');
        }]);

        $availablePigeons = Pigeon::where('user_id', $this->user->id)
            ->where('status', 'racing')
            ->whereNotIn('id', $this->season->entries->pluck('id'))
            ->select('id', 'ring_number', 'personal_number', 'name', 'gender', 'color', 'bloodline')
            ->get();

        // Should not throw database errors
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Collection::class, $this->season->entries);
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Collection::class, $availablePigeons);
        $this->assertCount(1, $this->season->entries);
        $this->assertCount(1, $availablePigeons); // Only pigeon2 is racing and not in season
        $this->assertEquals($pigeon2->id, $availablePigeons->first()->id);
    }
}
